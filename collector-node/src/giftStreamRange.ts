...
/** Resume after a completed month cursor (YYYYMMDD). */
export function resumeBegin(rangeBegin: string, cursor: string | null | undefined): string {
    if (!cursor || !/^\d{8}$/.test(cursor)) return rangeBegin;
    return maxYmd(addCalendarDays(cursor, 1), rangeBegin);
}
